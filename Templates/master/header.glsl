...
Ray ScreenRay()
{
    vec2 uv = (uFishEye>0.0) ? FishEyeUV(uFishEye) : (gl_FragCoord.xy/uResolution);
    Ray ray = ScreenRayUV(uv);
    ray.origin += uV[0].xyz * (texture(uImages[3], vec2(uBeats * uShake.x, uShake.y)).x - 0.5) * uShake.z;
    ray.origin += uV[1].xyz * (texture(uImages[3], vec2(uBeats * uShake.x, uShake.y) + 0.5).x - 0.5) * uShake.z;
    return ray;
}
...
{
	return pow(min(1.0 + dot(data.hit.normal, data.ray.direction), 1.0), power) * data.material.albedo * (1.0 - data.material.specularity) * color;
}
